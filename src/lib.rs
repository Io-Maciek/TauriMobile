...
use wry::webview::WebViewBuilderExtAndroid;
...
pub extern "C" fn start_app() {
	#[cfg(target_os = "android")]
	android_binding!(com_iogames, tam, _start_app);
	_start_app()
}
...
fn build_webview(event_loop: &EventLoopWindowTarget<()>) -> Result<WebView> {
	#[cfg(target_os = "windows")]
		let u = "wry://localhost/assets/index.html";
	#[cfg(not(target_os = "windows"))]
		let u = "file:///android_asset/index.html";
	...
	let webview = WebViewBuilder::new(window)?
		...
		// moj koment (android only) .with_asset_loader("wry://assets/index.html")
